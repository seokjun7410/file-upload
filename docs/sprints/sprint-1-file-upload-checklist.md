...
> 상태: 정책 REST API와 Axios GET·고정 PATCH·커스텀 POST 화면 구현 완료, 커스텀 DELETE 화면·파일 저장 미완료
>
> 기준 문서: [스프린트 1 확장자 차단 정책](sprint-1-file-upload-extension-policy.md), [스프린트 1 API 계약](sprint-1-file-upload-api.md)

현재 완료 범위는 `ExtensionPolicy` 단일 엔티티, fixed/custom 카탈로그 불변식, DB 제약, quota 기반 커스텀 최대 200개 등록, 고정 정책 초기화, 예제 기능 제거, 정책 REST API와 공통 오류 응답, Axios GET 기반 정책 조회 화면, 고정 정책 PATCH 화면, 커스텀 정책 POST 화면이다. 커스텀 DELETE 화면, multipart 파일 저장과 업로드 통합·수동 검증은 아직 완료되지 않았다.

## 1. 완료 판정 원칙

- [ ] 각 API의 URL, HTTP 메서드, 요청 형식, 응답 형식, 상태 코드를 [API 계약 문서](sprint-1-file-upload-api.md)와 일치시킨다.
- [ ] 각 체크박스는 코드와 자동화 테스트 또는 실제 화면 확인으로 근거를 남긴다.
- [ ] FE와 BE가 동일한 정규화·오류 해석·상태 표시 규칙을 사용한다.
...
- [ ] 커스텀 확장자 삭제를 Axios `DELETE /api/v1/extension-policies/custom/{extension}`으로 호출한다.
- [ ] 삭제 성공 시 목록에서 항목을 제거하고 완료 상태를 표시한다.
...
- [ ] 서버에서 업로드 파일의 확장자를 추출하고 저장된 정책으로 차단 여부를 다시 판정한다.
- [ ] 차단 파일은 파일 저장 로직을 실행하지 않고 `422`와 `BLOCKED_EXTENSION`을 반환한다.
- [ ] 허용 파일은 정해진 저장 위치에 서버 생성 파일명으로 저장하고 `201` 응답을 반환한다.
- [ ] 파일 없음·업로드 불가 요청을 `400`과 계약된 오류 코드로 구분한다.

#### FE

- [ ] 파일 선택·업로드를 `FormData`의 `file` 필드와 Axios `POST /api/v1/files`로 호출한다.
- [ ] `400`, `422` 및 공통 오류 JSON의 `message`를 사용자 메시지로 표시한다.
...
- [ ] 브라우저 개발자 도구에서 업로드 URL, 메서드, `file` 필드, 응답 상태 코드가 계약과 일치한다.
...
- [ ] 파일 업로드 API도 오류 상황별 상태 코드와 공통 오류 JSON을 계약에 맞게 반환한다.
